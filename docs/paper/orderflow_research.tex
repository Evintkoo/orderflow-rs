...
\usepackage{calligra}            % cursive signature font
...
{\large\bfseries\color{darkblue} Declaratio Originalitatis}

\vspace{0.15cm}
{\small\itshape\color{gray1} Declaration of Originality}

\vspace{1.0cm}

\noindent\textit{Ego, Evint Leovonzko, declaro atque testor hoc opus ---
analysin empiricam ordinum limitatorum in mercatis frequentiae altae
a capite usque ad calcem --- propria industria et ingenio composuisse.
Fontes omnes adhibiti in indice bibliographico enumerantur. Programmatum
fons (\code{orderflow-rs}), ratio colligendi data, omnesque analyses
empiricae a me ipso elaboratae sunt. Opus hoc alibi ad publicandum
nondum submissum est.}

\vspace{0.7cm}
\noindent\small\color{gray1}%
I, Evint Leovonzko, declare and attest that this work --- an empirical
analysis of high-frequency limit order book dynamics --- was composed
in its entirety by my own effort and intellect. All sources consulted
are listed in the bibliography. The \code{orderflow-rs} codebase, data
collection pipeline, and all empirical analyses were designed and
implemented by the author. This work has not been submitted elsewhere
for publication.
...
\textit{Subscripsit:} &
  {\calligra\Large Evint Leovonzko} \\[0.5cm]
\textit{Die:}         & 15 April 2025 \\
\end{tabular}
...
\label{eq:returns}
\end{equation}
These are stored as columns $r_{1\text{s}}$, $r_{5\text{s}}$,
$r_{30\text{s}}$, $r_{300\text{s}}$ in the feature CSV.
